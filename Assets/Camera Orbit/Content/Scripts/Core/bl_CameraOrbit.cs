...
public class bl_CameraOrbit : bl_CameraBase
{
    ...
    public void SetTarget(bl_OrbitTargetPlaceholder target)
    {
        ...
        IEnumerator TransitionToHolder()
        {
            ...
            {
                transform.position = Vector3.LerpUnclamped(transform.position, targetPosition, timer / duration);
                transform.rotation = Quaternion.SlerpUnclamped(transform.rotation, targetRotation, timer / duration);
                yield return null;
                timer += Time.deltaTime;
            }

            SetCameraToLocation(targetPosition, targetRotation);
